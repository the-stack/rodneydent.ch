<template>
  <svg class="map" :style="{top:svgTop}" viewBox="0 0 2560 5040" xmlns="http://www.w3.org/2000/svg"
       xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve">
    <use id="Hintergrund" xlink:href="#_Image1" x="0" y="0" width="2560px" height="5040px"/>
    <g transform="matrix(0.666667,0,0,0.666667,0,0)">
        <path class="map-route" ref="routemap" :style="{strokeDasharray: dashArray, strokeDashoffset: dashOffset}"
    ...
    <g class="cursor--pointer flag"
       v-show="scrollPercentage >=book.percentage" :transform="'matrix(0.4,0,0,0.4,'+ book.x+','+book.y+')'">
        <use :xlink:href="'#_flag'+book.frontmatter.id" x="0" y="0" width="443px" height="376px"/>
    </g>
      </a>

    <defs>
      <image id="_Image1" width="2560px" height="5040px" href="/karte.webp"/>
...
onMounted(() => {
  ...
  // pick as many coordinates from an coordinateArray as there are books in the data but the coordinates should only exist once
...
function setDashArray() {
  dashArray.value = routemap.value.getTotalLength();
  dashOffset.value = routemap.value.getTotalLength();
}
